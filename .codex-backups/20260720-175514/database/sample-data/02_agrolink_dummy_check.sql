...
SELECT e.table_name,
       ...
       CASE
           ...
           WHEN e.expected_scale IS NOT NULL
                AND NVL(c.data_scale, -1) <> e.expected_scale THEN 'SCALE_CHECK'
...
SELECT e.table_name,
       ...
       CASE
           ...
           WHEN c.validated <> 'VALIDATED' THEN 'NOT_VALIDATED'
           ELSE 'OK'
...
/* 상품 신고 FK가 실제 reports.product_id에서 products.product_id를
   참조하는지 확인한다. reference_status가 OK이면 정상이다. */
SELECT expected.constraint_name,
       fk_column.table_name,
       ...
       CASE
           WHEN fk.constraint_name IS NULL THEN 'MISSING'
           WHEN NVL(fk_column.column_name, '(NULL)') <> expected.column_name
               THEN 'FK_COLUMN_CHECK'
           WHEN NVL(referenced_constraint.table_name, '(NULL)')
                <> expected.referenced_table THEN 'TABLE_CHECK'
           WHEN NVL(referenced_column.column_name, '(NULL)')
                <> expected.referenced_column THEN 'COLUMN_CHECK'
           ELSE 'OK'
...
LEFT JOIN user_cons_columns fk_column
  ON fk_column.constraint_name = fk.constraint_name
 AND fk_column.table_name = fk.table_name
...
LEFT JOIN user_cons_columns referenced_column
  ON referenced_column.constraint_name = referenced_constraint.constraint_name
 AND referenced_column.table_name = referenced_constraint.table_name
...
UNION ALL
SELECT 'ORDER_ITEM_FARM_MISMATCH', COUNT(*)
FROM order_items oi
JOIN orders o ON o.order_id = oi.order_id
JOIN products p ON p.product_id = oi.product_id
WHERE p.farm_id <> o.farm_id
UNION ALL
...
UNION ALL
SELECT 'PRODUCT_REPORT_SELLER_MISMATCH', COUNT(*)
FROM reports r
JOIN products p ON p.product_id = r.product_id
JOIN farms f ON f.farm_id = p.farm_id
WHERE r.report_type = 'PRODUCT'
  AND r.reported_user_id <> f.seller_id
ORDER BY check_name;
